...
module lendpay::rewards_tests {
    ...
    #[test(admin = @lendpay, treasury_admin = @0x2, borrower = @0x3)]
    fun multiple_allocations_for_same_campaign_can_be_claimed_sequentially(
        admin: signer,
        treasury_admin: signer,
        borrower: signer,
    ) {
        ...
        assert!(campaigns::claimable_amount(1, borrower_addr) == 100, 209);
        campaigns::claim_campaign(&borrower, 1);
        assert!(lend_token::balance_of(borrower_addr) == 100, 210);
        assert!(campaigns::claimable_amount(1, borrower_addr) == 150, 211);

        campaigns::claim_campaign(&borrower, 1);

        assert!(lend_token::balance_of(borrower_addr) == 250, 212);
        assert!(campaigns::claimable_amount(1, borrower_addr) == 0, 213);
    }
}
